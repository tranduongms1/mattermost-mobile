...
export const patchChecklist = async (serverUrl: string, id: string, index: number, patch: any) => {
    try {
        const client = NetworkManager.getClient(serverUrl);
        const data = await client.doFetch(
            `/plugins/xerp/api/tasks/${id}/checklists/${index}`,
            {method: 'patch', body: patch},
        ) as any;
        return data;
    } catch (error) {
        logDebug('error on patchChecklist', getFullErrorMessage(error));
        forceLogoutIfNecessary(serverUrl, error);
        return {error};
    }
};

export const patchChecklistItem = async (serverUrl: string, id: string, checklistIdx: number, itemIdx: number, patch: any) => {
    try {
        const client = NetworkManager.getClient(serverUrl);
        const data = await client.doFetch(
            `/plugins/xerp/api/tasks/${id}/checklists/${checklistIdx}/items/${itemIdx}`,
            {method: 'patch', body: patch},
        ) as any;
        return data;
    } catch (error) {
        logDebug('error on patchChecklistItem', getFullErrorMessage(error));
        forceLogoutIfNecessary(serverUrl, error);
        return {error};
    }
};
