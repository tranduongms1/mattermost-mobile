...
const makeCloseButton = (theme: Theme) => {
    ...
    return buildNavigationButton(CLOSE_BUTTON_ID, 'close.create-trouble.button', icon);
};

const ATTACH_CAMERA_ID = 'attach-camera';
const ATTACH_IMAGE_ID = 'attach-image';
const CLOSE_BUTTON_ID = 'close-create-trouble';
...
function CreateTrouble({
    ...
    const onChange = useCallback((name: string, value: any) => {
        ...
        updateDraftProps(serverUrl, currentChannelId, 'trouble', {[name]: value});
    ...
    const addFiles = useCallback((newFiles: FileInfo[]) => {
        ...
        addFilesToDraft(serverUrl, currentChannelId, 'trouble', newFiles);

        for (const file of newFiles) {
            DraftUploadManager.prepareUpload(serverUrl, file, currentChannelId, 'trouble');
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            removeDraft(serverUrl, currentChannelId, 'trouble');
            Alert.alert('Bạn đã báo trouble thành công');
            close();
        } catch (e) {
