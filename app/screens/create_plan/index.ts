...
import {observeCanUploadFiles, observeConfigIntValue, observeCurrentChannelId, observeCurrentUserId, observeMaxFileCount} from '@queries/servers/system';
...
const enhanced = withObservables([], ({database}: WithDatabaseArgs) => {
    ...
    const draft = currentChannelId.pipe(
        switchMap((cId) => queryDraft(database, cId, 'plan').observeWithColumns(['files']).pipe(
...
export default withDatabase(enhanced(CreatePlan));
