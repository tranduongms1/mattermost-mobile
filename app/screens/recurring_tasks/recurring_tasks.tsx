...
const RecurringTasks = ({
    ...
    const openThread = useCallback(preventDoubleTap(() => {
        fetchAndSwitchToThread(serverUrl, id);
    }), [serverUrl]);
    ...
    const updateChecklist = useCallback(preventDoubleTap((index, data) => {
        if (!presssed.current) {
            presssed.current = true;
            patchChecklist(serverUrl, id, index, data).
                finally(() => {
                    presssed.current = false;
                });
        }
            ...
            <ScrollView style={styles.scrollView}>
                <View style={styles.card}>
                    <View style={styles.row}>
                        <Text style={styles.header}>{title || message}</Text>
                    </View>
                    {Boolean(end_date) &&
                    ...
                    {items.length > 0 &&
                    <View style={[styles.row, {paddingBottom: 12, paddingTop: end_date ? 2 : 12}]}>
                        <ProgressBar
                            color='#009AF9'
                            progress={progress}
