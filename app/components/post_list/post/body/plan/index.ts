...
const enhance = withObservables(['post'], ({post, database}: {post: PostModel} & WithDatabaseArgs) => {
    ...
    const troubles = post.props.troubles ? queryPostsById(database, post.props.troubles).observeWithColumns(['props']) : of$([]);
    const issues = post.props.issues ? queryPostsById(database, post.props.issues).observeWithColumns(['props']) : of$([]);
